class ConvertContributionsToMaterializedView < ActiveRecord::Migration
  def view_statement
    "SELECT CASE WHEN (pos.id IS NULL) THEN ((((per.project_id)::bigint << 32) + (per.name_id)::bigint) + (B'10000000000000000000000000000000'::\"bit\")::bigint) 
           ...
           CASE WHEN (pos.id IS NULL) THEN per.name_fact_id 
                ELSE (
                  SELECT name_facts.id FROM name_facts WHERE ((name_facts.analysis_id = p.best_analysis_id) AND (name_facts.name_id = pos.name_id))
                ) 
           END AS name_fact_id, 
           pos.id AS position_id 
         FROM (
           (people per LEFT JOIN positions pos ON (per.account_id = pos.account_id)) 
