...
describe 'ProjectBadgesController' do
  ...
  describe 'create' do
    it 'Should redirect if user is not logged in' do
      post :create, project_id: @project_id, project_badge: build(:project_badge, project: @project, repository: @repository, type: 'CiiBadge').attributes
    ...
    it 'Should not create ciibadge if url is not a empty' do
      login_as @account
      post :create, project_id: @project_id, project_badge: build(:project_badge, project: @project, repository: @repository, type: 'CiiBadge', url: '').attributes
    ...
    it 'Should create badge if user is logged in and correct badge param is passed' do
      login_as @account
      post :create, project_id: @project_id, project_badge: build(:project_badge, project: @project, repository: @repository, type: 'CiiBadge', url: '1').attributes
  ...
  describe 'destroy' do
    it 'Should not soft delete a badge if use is not logged in' do
      project_badge = create(:project_badge, project: @project, repository: @repository, type: 'CiiBadge', url: '1')
      create(:permission, target: @project, remainder: true)
      login_as create(:account)
    ...
    it 'should soft delete the badge if authorized user sends delete request' do
      ...
      delete :destroy, project_id: @project_id, id: project_badge.id
      flash[:success].must_be :present?
      project_badge.reload.deleted must_equal true
      must_redirect_to action: :index
    end
  end
end
