...
class PropertyEditTest < ActiveSupport::TestCase
  ...
  def setup
    @undone_edit = create(:property_edit, value: 'Spammy!', created_at: Time.now + 5.days,
                                          undone: true, undone_at: Time.now + 5.days, undone_by: accounts(:admin).id)
    @edit = create(:property_edit, value: 'Linux')
    @previous_edit = create(:property_edit, value: '456', created_at: Time.now - 5.days)
  end
  ...
  def test_undo_fails_with_no_editor
    ...
    @edit.target.reload
    assert_equal 'Linux', @edit.target.name
  end
  ...
  def test_undo_works_with_editor
    ...
    @edit.do_undo
    @edit.target.reload
    assert_equal '456', @edit.target.name
  end
  ...
  def test_redo_fails_with_no_editor
    ...
    assert_equal 'Linux', @undone_edit.target.name
  end
  ...
  def test_redo_works_with_editor
    ...
    @undone_edit.do_redo
    @undone_edit.target.reload
    assert_equal 'Spammy!', @undone_edit.target.name
  end
