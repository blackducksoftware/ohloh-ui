...
class PersonTest < ActiveSupport::TestCase
  ...
  describe 'searchable_factor' do
    it 'must return 0.0 when kudo_position is null' do
      ...
      _(person.searchable_factor).must_equal 0.0
    end
    ...
    it 'must return a significantly lower value when no account_id' do
      ...
      _(person.searchable_factor).must_be_close_to 0.099
    end
    ...
    it 'must return valid value' do
      ...
      person.kudo_position = 1
      _(person.searchable_factor).must_be_close_to 1.0, 0.01

      person.kudo_position = 5
      _(person.searchable_factor).must_be_close_to 0.42, 0.01

      person.kudo_position = 7
      _(person.searchable_factor).must_be_close_to 0.14, 0.01
    end
  end

  it 'should set id to account id or random' do
    _(account.person.id).must_equal account.id

    person = create(:person)
    assert person.valid?
    _((person.id > 2_147_483_647)).must_equal true
  end

  it 'should set effective_name to an account name or name' do
    _(account.person.effective_name).must_equal account.name

    person = create(:person)
    _(person.effective_name).must_equal person.name.name
  end
  ...
  it 'should not set name_fact when name_id is not present' do
    ...
    _(person.name_fact_id).must_be_nil
  end

  it 'should change effective_name when account name changed' do
    _(account.name).must_equal account.person.effective_name
    account.update(name: 'new_name_test')
    _(account.person.effective_name).must_equal 'new_name_test'
  end
  ...
  it '#find_claimed with sort option' do
    ...
    _(people.size).must_equal 2
    _(people[0].account).must_equal admin
    _(people[1].account).must_equal account
  end

  it '#find_claimed with search option' do
    people = Person.find_claimed(account.login, nil)
    _(people.length).must_equal 1
    _(people.size).must_equal 1
    _(people.first.account_id).must_equal account.id
  end
  ...
  it '#find_unclaimed with sort option' do
    ...
    people = Person.find_unclaimed
    _(people[0].last.first).must_equal person1
    _(people[1].last.first).must_equal person2
    _(people[2].last.first).must_equal person3
  end

  it '#find_unclaimed with search by name' do
    create(:person, effective_name: 'joe')
    create(:person)
    _(Person.find_unclaimed.length).must_equal 2
  ...
  it '#find_unclaimed search by multiple emails' do
    ...
    _(people.length).must_equal 1
    _(people.first.first).must_equal person.name_id
  end

  it 'should rebuild by project id' do
    create(:person, project_id: project.id)
    create(:person, project_id: project.id)
    _(Person.where(project_id: project.id).count).must_equal 2
  ...
  it 'must destroy existing person on creating position' do
    ...
    _(Person.find_by(id: person.id)).must_be_nil
  end
  ...
  it 'must create new person when position is destroyed' do
    ...
    _(Person.find_by(project: project, name: name)).must_be :present?
  end
  ...
  it 'must create new person when name is updated to null' do
    ...
    _(Person.find_by(project: position.project, name: name)).must_be :present?
  end

  it 'test_searchable_vector' do
    _(account.person.searchable_vector[:a]).must_equal "#{account.name} #{account.login}"
  end
  ...
  it 'find_claimed with search term' do
    ...
    _(people.length).must_equal 1
    _(people.size).must_equal 1
    _(people.first.account_id).must_equal admin.id
  end
  ...
  it 'find_unclaimed when destroying positions' do
    ...
    _(Person.find_unclaimed.count).must_equal 3
    ...
    people = Person.find_unclaimed
    _(people.length).must_equal 4
    _(people.map(&:last).flatten.any? { |p| p.name == person.name }).must_equal true
  end
  ...
  it 'find_unclaimed when deleting project' do
    ...
    _(Person.find_unclaimed.length).must_equal 2 # still 2 unique names
  end
