...
class Position::ValidationsTest < ActiveSupport::TestCase
  ...
  describe 'create' do
    it 'must check the existence of given project' do
      ...
      position.wont_be :valid?
      position.errors.messages[:project_oss].first.must_equal I18n.t('position.project_id.blank')
    end
    ...
    it 'wont allow project title longer than 100 chars' do
      ...
      long_string.size.must_equal 101

      position = build(:position, name_id: name_obj.id, title: long_string)

      position.wont_be :valid?
      position.errors.messages[:title].first.must_equal 'is too long (maximum is 100 characters)'
    end

    it 'wont allow description longer than 500 chars' do
      long_string = '0123456789' * 50 + 'a'
      long_string.size.must_equal 501

      position = build(:position, name_id: name_obj.id, description: long_string)

      position.wont_be :valid?
      position.errors.messages[:description].first.must_equal 'is too long (maximum is 500 characters)'
    end
    ...
    describe 'start_date' do
      ...
      it 'must be present when no committer_name' do
        ...
        error_message.must_equal i18n_activerecord(:position, 'committer_name.blank')
      end
      ...
      it 'wont be in the future' do
        ...
        error_message.must_equal I18n.t('position.start_date.in_future')
      end
    ...
    describe 'stop_date' do
      ...
      it 'wont be in the future' do
        ...
        error_message.must_equal I18n.t('position.stop_date.in_future')
      end
      ...
      it 'wont be earlier than start_date' do
        ...
        error_message = position.errors.messages[:stop_date].first
        error_message.must_equal I18n.t('position.stop_date.earlier')
      end
    end
    ...
    describe 'committer_name' do
      it 'wont allow creating a position without a valid committer_name' do
        ...
        position.wont_be :valid?
        position.errors.messages[:committer_name].first.must_equal I18n.t('position.no_name_fact')
      end
  ...
  it 'wont allow duplicate claims from different accounts' do
    ...
    new_position.wont_be :valid?

    error_message = new_position.errors.messages[:committer_name].first
    error_message.must_equal I18n.t('position.name_already_claimed', name: john.name)
  end
  ...
  it 'must not allow more than one position per project and account' do
    ...
    position.wont_be :valid?
    position.errors[:project_id].first.must_equal i18n_activerecord(:position, 'project_id.taken')
  end
end
