...
class BrokenLinkTest < ActiveSupport::TestCase
  ...
  describe '.filter_by' do
    it 'filter_by with nil string' do
      _(BrokenLink.filter_by(nil).count).must_equal BrokenLink.count
    end

    it 'filter_by with error type' do
      _(BrokenLink.filter_by('301: Net::HTTPMovedPermanently')).must_equal [broken_link_with_redirection]
    end
  end
end
