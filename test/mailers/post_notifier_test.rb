...
class PostNotifierTest < ActionMailer::TestCase
  ...
  test "user who originally created the first post for a topic should receive a post creation email" do
    ...
    assert_match "Hello #{accounts(:admin).name}, your post has been successfully created under #{topics(:sticky).title}", email.body.encoded
  ...
  test "user who replied should receive a post replied notification email" do
    ...
    assert_match "Hello #{user1.name}, #{user2.name} has replied to your post under #{topic.title}", email.body.encoded
    assert_not ActionMailer::Base.deliveries.empty?
    assert_equal 1, ActionMailer::Base.deliveries.size
  end
end