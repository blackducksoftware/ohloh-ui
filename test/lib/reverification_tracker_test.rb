    ...
    describe 'first notification phase' do
      ...
      describe 'success scenario' do
        ...
        it "should create a reverification tracker with 'initial' status" do
          rev_tracker = create(:reverification_tracker, account_id: @success_account.id)
          Account.stubs(:find_by_email).returns(@success_account)
          ReverificationTracker.stubs(:create_reverification_tracker).with(@success_account).returns(rev_tracker)
          ReverificationTracker.poll_success_queue
