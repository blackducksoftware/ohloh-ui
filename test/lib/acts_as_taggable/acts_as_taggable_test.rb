...
class ActsAsTaggable::ActsAsTaggableTest < ActiveSupport::TestCase
  it 'tag_with properly strips out whitespace' do
    project = create(:project)
    project.tag_with("   foo\t bar \r\n")
    project.tags.count.must_equal 2
    project.tags.pluck(:name).sort.must_equal %w(bar foo)
  end

  it 'tag_with properly strips out double quotes' do
    project = create(:project)
    project.tag_with('"real" beef')
    project.tags.count.must_equal 2
    project.tags.pluck(:name).sort.must_equal %w(beef real)
  end

  it 'tag_with leaves punctuation alone' do
    project = create(:project)
    project.tag_with('c++ SHA-224 Google_Apps i18n_(internationalization)')
