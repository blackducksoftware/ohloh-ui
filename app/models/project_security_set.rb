class ProjectSecuritySet < ActiveRecord::Base
  ...
  def release_history(release_ids = [])
    ...
    sql = <<-SQL
      select R.id, R.version, R.released_on, T.low, T.medium, T.high from releases R inner join ( select R.id, R.released_on,
      ...
      on RV.release_id = R.id full outer join vulnerabilities V on V.id = RV.vulnerability_id #{condition} group by R.id ) T on T.id = R.id order by R.released_on asc;
    SQL
