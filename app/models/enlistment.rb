class Enlistment < ActiveRecord::Base
  ...
  scope :by_last_update, -> { joins(code_location: :code_sets).joins('LEFT JOIN jobs ON enlistments.code_location_id = jobs.code_location_id')
                              .order('jobs.status, code_sets.updated_on DESC') }
