    ...
    def first_for_name_id_and_project_id(name_id, project_id)
      sql = <<-SQL
        SELECT name_facts.* FROM name_facts
          ...
          WHERE ( name_facts.name_id = #{name_id} OR name_facts.name_id IN (
            SELECT AA.preferred_name_id FROM analysis_aliases AA
            WHERE AA.analysis_id = projects.best_analysis_id AND AA.commit_name_id = #{name_id}))
          AND projects.id = #{project_id} AND type='ContributorFact'
        SQL
