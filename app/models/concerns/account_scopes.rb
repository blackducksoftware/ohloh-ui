module AccountScopes
  ...
  included do
    ...
    scope :unverified, -> (limit = nil) { select('accounts.id, accounts.email').joins("LEFT OUTER JOIN verifications v ON v.account_id = accounts.id").
                                    where("v.account_id IS NULL").limit(limit) }
    scope :reverification_not_initiated, -> (limit = nil) { unverified.joins('LEFT OUTER JOIN reverification_trackers r ON r.account_id = accounts.id').
