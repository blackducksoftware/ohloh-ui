class Account::OrganizationCore
  ...
  def orgs_for_my_positions
    @orgs_positions ||=
      Organization.active.joins { projects.positions }.where { positions.account_id.eq my { id } }
      .order { id }.distinct
  end
  ...
  def contributions_outside_org
    org_contributions_for.where do
      projects.organization_id.not_eq(accounts.organization_id) | projects.organization_id.eq(nil)
    end.count
  end
