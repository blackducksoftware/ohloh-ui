class Account::PositionCore < OhDelegator::Base
  ...
  def create_alias(project, name, existing_position, position_attributes)
    ...
    if alias_obj
      update_alias(alias_obj, existing_position.name_id, name.id)
    else
      alias_obj = Alias.create(project_id: project.id, commit_name_id: name.id,
                               preferred_name_id: existing_position.name_id, editor_account: account)
    end
