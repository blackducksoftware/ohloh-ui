...
module Api::VulnerabilitiesHelper
  ...
  def bdsa_cve_id(data)
    cve_id = data['_meta']['links'].select { |c| c['rel'] == 'cve' }[0]['href'].split('/').last
    cve_url = ENV['NVD_LINK'] + cve_id
    link_to(cve_id, cve_url, target: '_blank', rel: 'noopener')
  end

  def bdsa_cvss_severity(data)
    "#{data['temporalMetrics']['score']} #{data['severity'].titleize}"
  end

  def bdsa_cvss(cvss_data)
    [cvss_data['temporalMetrics']['score'],
     cvss_data['baseScore'],
