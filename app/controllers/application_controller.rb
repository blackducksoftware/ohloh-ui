class ApplicationController < ActionController::Base
  ...
  def find_claimed_persons(opts = {})
    opts = opts.reverse_merge(q: @q, page: params[:page])
    Person.find_claimed(opts)
  end

  def preload_claimed_persons(collection = nil, load_logos = true)
    collection ||= @claimed_persons
    all_position_ids = []
    ...
    @positions_map = Position.where{id.in all_position_ids.flatten}.includes(:project).references(:all).index_by(&:id)
    @logo_ids = @positions_map.values.map { |po| po.project.logo_id }.compact
