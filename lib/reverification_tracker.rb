  ...
  class << self
    def third_phase_accounts(limit)
       Account.find_by_sql("SELECT accounts.email FROM accounts
                          ...
                          AND reverification_trackers.status = 'spam' LIMIT #{limit}") 
    end
    ...
    def poll_transient_bounce_queue
      ...
      transient_bounce_queue.poll(initial_timeout: 1, idle_timeout: 1) do |msg|
        ...
        elsif account.reverification_tracker.present? && account.reverification_tracker.status == 'spam'
          ses.send_email(one_day_before_deletion_notice(email_address))
    ...
    def update_reverification_tracker(account)
      if account.reverification_tracker.status == 'spam'
        account.reverification_tracker.update(status: 'final warning', updated_at: DateTime.now.utc)
      elsif account.reverification_tracker.status == 'marked for spam'
    ...
    def time_is_right?(created_at, updated_at, status)
      if status == 'spam' || status == 'marked for spam'
    ...
    def send_one_day_left_before_deletion_notification
      return if ses_limit_reached?
      third_phase_accounts(5).each do |account|
        ...
        time_is_right?(created_at, updated_at, status) ? ses.send_email(one_day_before_deletion_notice(account.email)) : return
      end
    end
